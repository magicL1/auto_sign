...
任务默认每天北京时间 11:33 执行。手动触发默认是只读演练，只有勾选“正式执行账户操作”才会产生账户变更。
...
- 只读 GET 请求遇到临时网络错误时最多重试 3 次；签到、免费抽奖和关注操作通过 Playwright 点击官网页面控件，由网页生成动态校验参数，不直接调用需要动态签名的写接口。
- Actions 使用免费的 Xvfb 虚拟显示器运行有界面 Chromium，避免掘金页面在无界面模式下缺失交互控件；仍不需要个人电脑开机。
- API 请求通过 Playwright 的独立请求上下文发送并与页面共享 Cookie，避免浏览器跨域预检拦截。
